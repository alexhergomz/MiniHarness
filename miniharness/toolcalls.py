...
Vendored from Promethean (`tool_call_recovery.py`) — one of the three pieces
worth carrying over verbatim. Pure, provider-agnostic, no dependencies.

Many local models (Qwen-Coder and friends, served via llama.cpp or Ollama) do
not populate the OpenAI ``message.tool_calls`` array. Instead they write the
